...
public class UaUserLayer {
	...
	public UaUserLayer(String userSip, int listenPort, String proxyAddress, int proxyPort)
		...
		this.sipUri = userSip;
		this.proxyDomain = userSip.split("@")[1];
	}
	...
	public void startListeningNetwork() {
		try {
			// Creamos el mensaje REGISTER
			RegisterMessage register = createRegisterMessage();
			transactionLayer.call(register);
		} catch (IOException e) {
	...
	private RegisterMessage createRegisterMessage() {
		System.out.println("Sending REGISTER to proxy...");
		RegisterMessage register = new RegisterMessage();
		String fromName = sipUri.split("@")[0]; // ej. "alice"

        // Destino (El proxy/dominio)
        register.setDestination("sip:" + proxyDomain);
        
        // Via (Mi IP y puerto)
        register.setVias(new ArrayList<String>(Arrays.asList(myAddress + ":" + listenPort)));
        register.setMaxForwards(70);
        
        // To (Mi propia URI)
        register.setToName(fromName);
        register.setToUri("sip:" + sipUri);

        // From (Mi propia URI)
        register.setFromName(fromName);
        register.setFromUri("sip:" + sipUri);

        register.setCallId(UUID.randomUUID().toString());
        register.setcSeqNumber("1");
        register.setcSeqStr("REGISTER");
        
        // Contact (Dónde pueden encontrarme)
        register.setContact(myAddress + ":" + listenPort);
        register.setExpires("3600"); // 1 hora
